$(function() {
	$("#main-menu").find("li.board").click(function() {
		$("#lnb").toggleClass("show")
	})

	$("#main-menu").find(">li.board >div#lnb").hide();
	$("#main-menu").find(">li.show >div#lnb").show();
...
$(function() {
	...
	$lnb.find(">li").filter(":last").find("a:last").off('keydown').on(
			'keydown', function(b) {
	...
	$lnb.find(">li").filter(":last").find(">a").off('keydown')
	.on(
			'keydown',
			function(b) {
				if (b.keyCode == 9 && b.shiftKey) {
				} else if (b.keyCode == 9) {
					if ($(this).parent("li").find(">ul").css(
					"display") == "none") {
						lnbReset();
					}
				}
	...
	$lnb.find(">li").filter(":first").find(">a").off('keydown').on(
			'keydown', function(b) {
				if (b.keyCode == 9 && b.shiftKey) {
					lnbReset();
				} else if (b.keyCode == 9) {
	...
	$lnb.find(">li>ul>li>ul>li").filter(":last").find("a").off('keydown')
